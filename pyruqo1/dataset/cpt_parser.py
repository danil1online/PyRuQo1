...
class CPTParser:
    """Парсинг PDF для CPT (Continual Pre-Training): извлечение сырого текста с LaTeX через Marker."""
    ...
    def parse_folder(self, folder_path: str, recursive: bool = True) -> List[str]:
        ...
        self.logger.info(f"CPT: найдено {len(pdf_files)} PDF-файлов.")

        all_text = []
        for file_path in tqdm(pdf_files, desc="CPT парсинг"):
            text = self.parse_pdf(file_path)
            if text:
                all_text.append(text)

        self.logger.info(f"CPT: извлечено текста из {len(all_text)} файлов.")
        return all_text
