...
class DatasetGenerator:
    ...
    def generate_from_chunks(
        ...
        format_instruction = (
            ...
            "Пиши ответ исключительно как независимый эксперт на основе своих собственных глубоких знаний. "
            "Представь, что тебе задали этот вопрос на научном симпозиуме. "
            "КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО упоминать, что ты читаешь текст, фрагмент, тезисы, статью или базис. "
            "Никогда не пиши: 'в приведенном тексте', 'автор указывает', 'в тексте указано', 'для ответа на вопрос следует проанализировать текст'. "
            "Сразу начинай рассуждение с сути дела: формулируй концепции, законы и логические связи от первого лица, интегрируя предоставленные факты как свои собственные мысли.\n"
        ...
        user_prompt_template = "Базовые академические тезисы и факты для интеграции в ответ: с LaTeX-формулами:" if mode == "math" else "Базовые академические тезисы и факты для интеграции в ответ:"
                ...
                try:
                    ...
                    if full_response:
                        ...
                        parasite_patterns = [
                            ...
                            r"На основе (?:приведенного|представленного) текста,?\s*",
                            r"В статье (?:подчеркивается|указывается|отмечается), что\s*",
                            r"Автор (?:указывает|отмечает|предлагает), что\s*",
                            r"Из (?:приведенного|данного) фрагмента текста видно, что\s*",
                        ]
