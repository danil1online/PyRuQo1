...
class DatasetGenerator:
    ...
    def generate_from_chunks(
        ...
        if self.context_size == 2048:
            length_instruction = (
                "Пиши рассуждения лаконично, без повторений. "
                "Итоговый ответ в блоке <output> должен быть очень кратким и содержательным. "
            )
        else:  # 8192
            length_instruction = (
                "Проведи полную декомпозицию вопроса, сопоставь все факты в блоке <Thought>. "
